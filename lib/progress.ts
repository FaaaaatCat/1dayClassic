 ...
 * 홈의 읽기 버튼이 무엇을 할지.
 *
 * - first   한 화도 끝내지 못한 책 — '무료로 첫화보기'
 * - resume  마지막으로 끝낸 것의 다음 화 — '이어 읽기 : 제n화'
 * - restart 무료로 열린 마지막 화까지 끝냈다 — '1화부터 다시 읽기'
 */
 ...
 *
 * 기준은 '마지막으로 끝낸 화' 하나다 — 그 바로 다음을 연다.
 *
 * 1. 한 화도 끝내지 못했으면 첫 화 — '무료로 첫화보기'.
 * 2. 끝낸 것이 있으면 그 다음 화 — '이어 읽기 : 제n화'.
 * 3. 마지막으로 끝낸 것이 무료로 열린 마지막 화면 더 갈 곳이 없다 — '1화부터 다시 읽기'.
 *    거기서 1화를 끝내면 커서가 1화로 옮겨 가고, 다시 2번 규칙에 따라 '제2화'가 된다.
 *
 * '끝냈다'의 기준은 CardDeckDetail이 정한다 — 퀴즈를 다 풀었거나, '다음 화 읽기'를
 * 눌렀거나, 마지막 장까지 닿았거나. 여는 것만으로는 끝이 아니라서, 읽다 나온 화는 다음
 * 번에도 그 화가 다시 열린다.
 *
 * 퀴즈 기록만 보고 정하지는 않는다. 다 푼 뒤 다시 읽을 때는 진도가 전부 '풀었음'이라
 * 앞뒤를 가릴 수 없고, 여기가 답해야 하는 물음은 '어디를 안 풀었나'가 아니라 '어디까지
 * 끝냈나'이기 때문이다. 퀴즈 기록은 진도·정답률이 계속 맡는다.
 *
 * 잠긴 항목은 고르지 않는다. 그러지 않으면 무료로 열린 마지막 화를 끝낸 순간 이 버튼이
 * 잠긴 다음 화를 열어 목차의 자물쇠를 우회한다.
 *
 * @param cursorLessonId 그 책에서 마지막으로 끝낸 항목(ReadingCursorContext).
 */
export function getReadPlan(bookId: BookId, cursorLessonId?: string): ReadPlan | undefined {
  ...
  // 콘텐츠가 바뀌어 커서가 가리키던 항목이 사라졌으면 못 끝낸 것으로 친다.
 ...
 *
 * 홈의 읽기 버튼(getReadPlan)과 가리키는 곳은 대개 같다 — 방금 끝낸 항목이 곧 커서라서다.
