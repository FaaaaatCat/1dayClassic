...
## 이후 변경 (2026-08-07, 같은 날 늦게)

이 검증 이후 알람 표시 방식이 바뀌었다. 위 표의 시나리오 6("잠금 없을 때 알람 끄기")은
당시 헤드업 알림 경로를 전제로 측정한 것이고, 지금은 **오버레이 권한이 있으면 기기를 쓰는
중에도 전체화면이 뜬다.** 자세한 내용은 `2026-07-28-native-alarm-experience-design.md`의
「전체화면 표시 조건」을 볼 것.

잠금화면 위 플로우 자체(시나리오 1~5, 7)는 영향받지 않는다 — 잠금 상태의 표시 경로는
건드리지 않았다.
