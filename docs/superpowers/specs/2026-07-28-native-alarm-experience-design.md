...
기본 시계 앱과의 **유일한 차이점**: '끄기'를 누르면 알람만 종료되는 게 아니라 앱의 '오늘의 곡'
화면으로 이동해 자동 재생된다.
...
[Native] AlarmRingingService (Foreground, mediaPlayback)
   ...
   화면 꺼짐/잠금 ──→ AlarmActivity 전체화면
      │  [끄기]   → 서비스 종료 + MainActivity를 today/autoplay로 실행
      │  [스누즈] → 서비스 종료 + 5분 뒤 1회성 알람 예약
      │
   기기 사용 중   ──→ 헤드업 알림 (현재 작업 방해하지 않음)
         [끄기]   → 서비스 종료만 (앱을 열지 않는다)
         [스누즈] → 서비스 종료 + 5분 뒤 1회성 알람 예약
```

**끄기가 앱을 여는 것은 전체화면 경로에서만이다.** 헤드업 알림은 사용자가 다른 일을 하고 있다는
뜻이므로, 알림의 [끄기]는 소리만 멈추고 화면을 빼앗지 않는다.
...
```
   ...
   [ 스누즈 ]   [  끄기  ]   하단 버튼 (--beige-100)
```

버튼 동작 — 액티비티는 서비스에 인텐트만 보내고 자신은 `finish()`:

- **스누즈** → 서비스에 `ACTION_SNOOZE` 전달 → `finish()`
- **끄기** → 서비스에 `ACTION_DISMISS` 전달 → `MainActivity` 실행 → `finish()`
...
Intent(this, MainActivity::class.java).apply {
    ...
    data = Uri.parse("1dayclassic://today?autoplay=${System.currentTimeMillis()}")
    addFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_SINGLE_TOP)
}
```

expo-router가 이 딥링크를 받아 `/today` 화면으로 이동하고, `autoplay` 파라미터가 매번 새 값이라
기존 `today.tsx`의 자동 재생이 반복 알람에서도 매번 새로 트리거된다 (현재 동작과 동일).
