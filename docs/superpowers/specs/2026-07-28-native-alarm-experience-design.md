...
>
> **2026-09-04 갱신 — 위 3번을 되돌렸다**
>
> '다른 앱 위에 표시'(SYSTEM_ALERT_WINDOW) 권한을 걷어냈다. 다른 앱을 쓰는 중이라면
> 전체화면으로 덮지 않기로 했기 때문이다. 그래서 `AlarmReceiver`의 `startActivity()` 직접
> 실행도, 헤드업을 억제하던 조용한 채널도 함께 사라졌다.
>
> 이제 화면을 띄우는 길은 `setFullScreenIntent()` 하나다 — 잠금·화면꺼짐이면 전체화면,
> 기기 사용 중이면 헤드업 알림. 소리는 어느 쪽이든 그대로 울린다. 권한도 넷에서 셋으로
> 줄었다. 아래 본문은 그에 맞춰 고쳤다.
...
**깨우는 자리는 잠든 사이다.** 그때는 기기가 잠겨 있거나 화면이 꺼져 있으므로 전체화면이 뜬다.
반대로 다른 앱을 쓰고 있는 중이라면 하던 일을 통째로 덮지 않는다(2026-09-04 변경).

길은 하나다 — **`setFullScreenIntent()`**. 기기가 잠겨 있거나 화면이 꺼져 있으면 액티비티를
실행하고, 사용 중이면 OS가 헤드업 알림으로 격하시킨다. 권한 없이도 동작하는 공식 경로다.

| 기기 상태 | 표시 방식 |
|---|---|
| 화면 꺼짐 / 잠금화면 | **전체화면** (FSI) |
| 기기 사용 중 | 헤드업 알림 (끄기·스누즈 액션 포함) |

어느 경우든 **소리는 동일하게 재생되고, 사용자가 끄기/스누즈를 누를 때까지 지속된다.**

#### BAL(Background Activity Launch)에 대하여

이 대목은 왜 `startActivity()` 직접 실행을 쓰지 않는지에 대한 기록으로 남긴다. 실측으로
확인한 것(SM-S942N, Android 16):
...
- 오버레이 권한이 있으면 `BAL_ALLOW_NON_APP_VISIBLE_WINDOW`로 통과한다. 알라미가 이 권한을
  요구하는 이유가 이것이다. 2026-08-07~09-04 사이에는 이 길을 썼다.

**그 길을 버렸다.** 얻는 것(다른 앱을 쓰는 중에도 전체화면)보다 치르는 값(사용자가 직접 켜야
하는 권한 하나 + 하던 일을 덮는 것)이 크다고 봤다. 지금은 잠금·화면꺼짐일 때만 전체화면이 뜨고,
그건 `setFullScreenIntent()`가 권한 없이 해 준다.

#### 알림 채널

채널은 하나다 — `alarm-ringing` (HIGH + FSI).

예전에는 둘이었다. 전체화면을 직접 띄우는 동안에는 헤드업까지 겹쳐 같은 알람이 두 번 보여서,
그때만 `alarm-ringing-quiet`(LOW)로 낮췄다. 직접 실행을 걷어내면서 겹칠 일이 없어졌다.

이제 이 알림 하나가 두 몫을 한다 — 잠금·화면꺼짐이면 FSI가 알람 화면을 띄우고, 기기를 쓰는
중이면 헤드업으로 떠서 끄기·스누즈를 내민다.
...
`SYSTEM_ALERT_WINDOW`는 2026-08-07에 더했다가 2026-09-04에 걷어냈다. Expo가 기본으로 넣는
권한이라 지우려면 `app.json`의 `blockedPermissions`에 적어야 한다 — `/android`는 prebuild가
다시 만들어서 거기서 지우면 되살아난다.
...
누른 사용자는 켤 때마다 같은 팝업을 다시 봤다. 사용자가 직접 켜야 하는 권한이 목록에 들어오면서
이 문제가 드러났다 — 거부한 사용자는 영원히 시달리게 된다.
...
**셋 중 시스템 팝업을 띄울 수 있는 것은 알림뿐이다.** 나머지 둘은 Android가 요청 API를 제공하지
않아 해당 권한의 설정 화면을 여는 것이 앱이 할 수 있는 전부다. 알라미도 같다.

그래서 이 셋을 하나로 묶을 수 없다 — 알림만 런타임 권한이고, 나머지 둘은 각각 다른 '특수 앱
액세스' 화면이다. 대신 개수는 줄 수 있다: `USE_EXACT_ALARM`과 `USE_FULL_SCREEN_INTENT`는
알람이 본질인 앱에 설치 시 자동 부여되므로, Play가 이 앱을 알람 앱으로 인정하면 사용자가
직접 켤 것은 알림 하나만 남는다.
...
알림이 셋 중 가장 치명적이다 — 없으면 소리는 나는데 끄고 스누즈할 자리가 화면에 없다.
...
2. 앱이나 다른 앱을 쓰는 중일 때 → **헤드업 알림으로 뜨고 전체화면이 덮지 않는지**
   (헤드업에 끄기·스누즈가 함께 있어야 한다)
