...
새 디자인(Figma)에서는 상세 화면이 7종 블록의 조합으로 정리됐고, **책마다 들어가는 블록과 그 순서가
달라진다.** 지금 구조로는 순서를 책마다 바꿀 수 없다 — 순서가 화면 파일에 하드코딩돼 있기 때문이다.

## 목표

- 상세 화면을 7종 블록 컴포넌트로 분해한다.
...
- 퀴즈 블록 구현 — 나중에 붙는다. 이 문서는 붙일 자리만 확인해 둔다.
...
특히 나중에 붙일 퀴즈는 책마다 형식이 다를 가능성이 커서(객관식/주관식/듣고 맞히기) 닫힌 유니온이
부담이 된다. JSX 조합은 **임의의 컴포넌트를 넣을 수 있어 블록 목록이 닫혀 있지 않다.**
...
기능 블록이라서가 아니다. 나중의 퀴즈 블록은 콘텐츠(문항)와 동작(채점·저장)을 둘 다 가지므로
`<QuizBlock questions={lesson.quiz} />`처럼 props를 받으면서 내부에서 `useQuiz()`를 쓴다.
...
- **영속 기능 상태** → 기능별 앱 Context (`LikesContext`, `NotesContext`, 나중에 `QuizContext`)

퀴즈가 들어올 때 `QuizContext`는 **추가만** 되고 위 둘은 수정되지 않는다.
...
이 설계는 퀴즈를 지금 만들지 않지만 자리는 확인해 뒀다.

- 문항 데이터는 책별 타입에 `quiz?: QuizQuestion[]`를 더한다. 기존 8권은 영향이 없다(optional).
- 조합 파일에 `<QuizBlock questions={lesson.quiz} />` 한 줄을 원하는 위치에 넣는다.
- 풀이 상태는 `QuizContext`가 갖는다. `NotesContext`가 이미 쓰고 있는 '항목 id로 갈라 AsyncStorage에
  영속화' 패턴을 그대로 따른다.
- 책마다 퀴즈 형식이 다르면 그 책 조합 파일에서 다른 컴포넌트(`<HanjaQuizBlock />`)를 쓰면 된다.
  블록 목록이 닫혀 있지 않아서 가능하다.
