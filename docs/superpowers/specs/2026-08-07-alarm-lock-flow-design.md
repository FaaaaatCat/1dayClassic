...
- 광고 화면(`/ad`)을 이 플로우에 포함하기 — 잠금화면 위에서 건너뛸 수 없는 광고가 되기
  때문이다. 화면과 라우트는 살려 두되 **어디에도 연결하지 않는다.** 붙일 자리는 아직
  정하지 않았다(2026-08-07 기준). 붙일 때는 잠금 플로우 중인지를 `useAlarmLockFlow()`로
  걸러야 같은 문제가 되풀이되지 않는다
- iOS — Android 전용
