 ...
 * 무채색 위에 얹는 두 색.
 *
 * ember는 이 앱의 포인트 컬러다. UI에 쓴다 — 지금 눌러야 할 버튼, 지금 어디까지 왔는지
 * 말하는 자리(완독바·진도 막대), 지금 이 책이라고 알리는 표식. 다만 '포인트'라는 말이
 * 조건이다: 한 화면에 여러 곳이 동시에 주황이면 어디를 보라는 말인지 사라진다. 화면마다
 * 가장 중요한 것 하나에만 얹고, 나머지는 무채색으로 둔다.
 *
 * violet은 여전히 그림 안에서만 쓴다 — 버튼·링크·배지·테두리에 쓰지 않는다.
 * (잠금화면 알람은 어두운 사진 위라 별도 계열인 Colors.violet*을 쓴다.)
 */
export const Spark = {
  violet: "#0447FF",
  ember: "#FF4704",
