...
function DeckCard({
  ...
  return (
    <Animated.View
      style={[styles.hinge, { zIndex: total - index }, hingeStyle]}
...
const styles = StyleSheet.create({
   ...
   *
   * 높은음자리표는 글자 하나가 위아래로 아주 길다. 글꼴이 정한 줄 상자보다 먹이 더 커서,
   * 줄높이를 비워 둬도(글꼴에 맡겨도) 위 갈고리와 아래 꼬리가 잘렸다. 그래서 줄 상자를
   * 표식 자리 높이(100)만큼 직접 열어 주고, 글자 크기는 그 안에 먹이 다 들어오도록
   * 줄인다 — 60이면 대략 84 높이라 위아래로 여덟씩 남는다.
   *
   * 크게 그리는 것보다 온전히 그리는 것이 먼저다.
   */
  coverGlyph: {
    fontSize: 60,
    lineHeight: SYMBOL_BOX_H,
