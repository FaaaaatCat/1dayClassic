 ...
 * 보고 나간 것과 끝까지 읽은 것을 그것으로 가른다. 읽은 시각도 그 기록(QuizAttempt.at)에
...
export function useBookStats(bookId?: BookId): BookStats {
  ...
  return useMemo(() => {
    ...
    for (const day of lessons) {
      ...
      quizTotal += 1;
      if (attempt.correct) quizCorrect += 1;
    ...
    const progress = getReadingProgress(bookId, (id) => attemptOf(id) !== undefined);
  ...
  }, [bookId, attemptOf, countOf, notesOf]);
}
